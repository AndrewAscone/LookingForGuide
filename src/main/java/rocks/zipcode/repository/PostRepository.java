...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(
        value = "select distinct post from Post post left join fetch post.activity left join fetch post.user",
        countQuery = "select count(distinct post) from Post post"
    )
    Page<Post> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct post from Post post left join fetch post.activity left join fetch post.user")
    List<Post> findAllWithToOneRelationships();

    @Query("select post from Post post left join fetch post.activity left join fetch post.user where post.id =:id")
