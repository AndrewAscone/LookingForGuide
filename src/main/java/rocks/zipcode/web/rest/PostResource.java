...
/**
 * REST controller for managing {@link rocks.zipcode.domain.Post}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Post> partialUpdatePost(
        ...
        Optional<Post> result = postRepository
            ...
            .map(existingPost -> {
                ...
                if (post.getGuideType() != null) {
                    existingPost.setGuideType(post.getGuideType());
                }
                ...
                if (post.getUserType() != null) {
                    existingPost.setUserType(post.getUserType());
                }
